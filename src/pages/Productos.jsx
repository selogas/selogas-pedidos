    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 max-h-[80vh] flex flex-col">
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={handleCrear} disabled={saving || !nuevaNombre.trim()} className="px-3 py-2 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50">
        ...
        <div className="overflow-y-auto flex-1 space-y-1">
          ...
          {categorias.map(cat => (
            <div key={cat.id} className="flex items-center gap-2 p-2 rounded-xl hover:bg-gray-50 border border-gray-100 group">
              {editandoId === cat.id ? (
                <>
                  <input autoFocus type="text" value={editNombre} onChange={e => setEditNombre(e.target.value)} onKeyDown={e => { if (e.key === "Enter") guardarEdicion(cat.id); if (e.key === "Escape") cancelarEdicion(); }} className="flex-1 border border-[#00c254] rounded-lg px-2.5 py-1 text-sm focus:outline-none bg-[#edf7f2]" />
                  <button onClick={() => guardarEdicion(cat.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg flex-shrink-0"><Check size={14} /></button>
                ...
                <>
                  ...
                  <button onClick={() => iniciarEdicion(cat)} className="p-1.5 text-gray-400 hover:text-[#00913f] hover:bg-[#edf7f2] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"><Pencil size={13} /></button>
                  <button onClick={() => handleEliminar(cat)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"><Trash2 size={13} /></button>
                </>
              )}
            </div>
          ))}
...
function CambiarGrupoModal({ productos, onClose, onChanged }) {
  ...
  useEffect(() => {
    supabase.from("tiendas").select("id, nombre").eq("activa", true).neq("nombre", "PRINCIPAL").order("nombre")
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[90vh] flex flex-col gap-3">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {GRUPOS.map(g => (
              <button key={g.value} onClick={() => setGrupoDestino(g.value)} className={`flex-1 py-2 rounded-xl border-2 text-sm font-bold transition-all ${grupoDestino === g.value ? "border-[#00913f] bg-[#edf7f2] text-[#00913f]" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>{g.label}</button>
            ))}
          </div>
        </div>
        <div className="grid grid-cols-2 gap-2">
          ...
          <div className="relative">
            ...
            <input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar..." className="w-full border rounded-xl pl-8 pr-3 py-2 text-sm focus:outline-none focus:border-[#00913f]" />
          </div>
        </div>
        <div className="flex items-center justify-between">
          <span className="text-xs text-gray-500">{productosFiltrados.length} visibles · <span className="font-semibold text-[#00913f]">{seleccion.size} seleccionados</span>{seleccion.size > 0 && <button onClick={() => setSeleccion(new Set())} className="ml-2 text-red-400 hover:text-red-600 underline">limpiar</button>}</span>
          <button onClick={toggleVisibles} className="text-xs text-[#00913f] hover:underline">{todosVisiblesSeleccionados ? "Deseleccionar visibles" : "Seleccionar visibles"}</button>
        </div>
        ...
        {grupoDestino === "especifico" && (
          <div className="border rounded-xl p-3 bg-amber-50 border-amber-200">
            <p className="text-xs font-semibold text-amber-800 mb-2">🎯 Selecciona las tiendas que verán estos productos:</p>
            <div className="grid grid-cols-2 gap-1.5 max-h-36 overflow-y-auto">
              {tiendas.map(t => (
                <label key={t.id} className="flex items-center gap-2 text-xs cursor-pointer hover:bg-amber-100 rounded-lg p-1.5">
                  <input type="checkbox" checked={tiendasSeleccionadas.has(t.id)} onChange={() => { setTiendaSel(prev => { const next = new Set(prev); if (next.has(t.id)) next.delete(t.id); else next.add(t.id); return next; }); }} className="rounded flex-shrink-0" />
                  <span className="truncate font-medium">{t.nombre}</span>
                </label>
              ))}
            </div>
            {tiendasSeleccionadas.size > 0 && <p className="text-xs text-amber-700 mt-1.5 font-semibold">{tiendasSeleccionadas.size} tienda{tiendasSeleccionadas.size > 1 ? "s" : ""} seleccionada{tiendasSeleccionadas.size > 1 ? "s" : ""}</p>}
          </div>
        )}
        <div className="flex gap-3">
          ...
          <button onClick={handleGuardar} disabled={saving || !seleccion.size || !grupoDestino || (grupoDestino === "especifico" && tiendasSeleccionadas.size === 0)} className="flex-1 py-2.5 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50 flex items-center justify-center gap-2">
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[90vh] flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <select value={origen} onChange={e => setOrigen(e.target.value)} className="w-full border rounded-xl px-3 py-2 text-sm">
        ...
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 py-2.5 border rounded-xl text-sm font-medium hover:bg-gray-50">Cancelar</button>
          <button onClick={handleMover} disabled={moving || !seleccion.size || !destino} className="flex-1 py-2.5 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50 flex items-center justify-center gap-2">
...
function ProductoModal({ producto, categorias, onClose, onSave, modo }) {
  const [form, setForm] = useState(producto ? {
    ...
    grupo_visualizacion: producto.grupo_visualizacion||'ambos',
    hoja_excel: producto.hoja_excel||'',
  } : {
    nombre:'', codigo:'', categoria_id:'', formato:'', multiplo:1, imagen_url:'',
    descripcion:'', disponible:true, grupo_visualizacion:'ambos', hoja_excel:'',
  });
  const [saving, setSaving]               = useState(false);
  ...
  const handleSave = async () => {
    ...
    const payload = {
      ...
      hoja_excel: form.hoja_excel || null,
        ...
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400 mt-1">P&aacute;gina donde aparecer&aacute; este producto en el PDF del pedido</p>
            </div>
            ...
            {form.grupo_visualizacion === 'especifico' && (
              <div className="border-2 border-amber-200 bg-amber-50 rounded-xl p-3">
                <p className="text-xs font-bold text-amber-800 mb-2">🎯 Tiendas que pueden ver este producto{tiendasSelec.size > 0 && <span className="ml-2 bg-amber-200 text-amber-900 px-2 py-0.5 rounded-full">{tiendasSelec.size} seleccionadas</span>}</p>
                ...
                {tiendasSelec.size === 0 && <p className="text-xs text-amber-600 mt-1.5">⚠️ Ninguna seleccionada — el producto no será visible para nadie</p>}
              </div>
            )}
