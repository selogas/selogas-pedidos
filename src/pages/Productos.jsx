...
import { Search, Package, Loader2, Pencil, ToggleLeft, ToggleRight, Plus, Trash2, X, Upload, ChevronRight, FolderOpen, Tag, Check, Layers, FileText } from "lucide-react";
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-5 border-b"><h2 className="font-bold text-lg">Subir im&aacute;genes por c&oacute;digo</h2><button onClick={onClose} className="p-2 hover:bg-gray-100 rounded-lg"><X size={18} /></button></div>
        <div className="flex-1 overflow-y-auto p-5">
          {fase === "inicio" && (
            <div>
              <p className="text-sm text-gray-500 mb-5">El <strong>nombre de cada archivo</strong> debe ser el <strong>c&oacute;digo del producto</strong> (ej: <code className="bg-gray-100 px-1.5 py-0.5 rounded text-xs">856607.jpg</code>).</p>
              <div className="border-2 border-dashed border-gray-300 rounded-2xl p-12 text-center hover:border-[#00c254] cursor-pointer transition-colors" onClick={() => inputRef.current?.click()}>
                ...
                <div className="inline-flex items-center gap-2 px-5 py-2.5 bg-[#00913f] text-white rounded-xl font-bold text-sm mt-3"><FolderOpen size={16} /> Elegir carpeta</div>
              </div>
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 gap-2 max-h-64 overflow-y-auto border rounded-xl p-3">
                {archivos.map((item, i) => (
                  <div key={i} className={`rounded-xl border-2 overflow-hidden ${item.ok ? "border-green-300" : "border-amber-300"}`}>
                    ...
                    <div className="p-1.5"><p className="text-xs font-bold truncate">{item.cod}</p><p className={`text-xs truncate ${item.producto ? "text-green-700" : "text-amber-600"}`}>{item.producto ? "✓ " + item.producto.nombre : "⚠ Sin coincidencia"}</p></div>
                  </div>
                ))}
              </div>
            </div>
          )}
          {fase === "subiendo" && (<div className="text-center py-10"><Loader2 size={40} className="animate-spin mx-auto mb-3 text-[#00913f]" /><p className="font-bold text-gray-700 mb-4">Subiendo im&aacute;genes... {progreso}%</p><div className="w-full bg-gray-100 rounded-full h-3"><div className="bg-[#00913f] h-3 rounded-full transition-all" style={{ width: progreso + "%" }} /></div></div>)}
          {fase === "hecho" && (<div><div className="grid grid-cols-2 gap-3 mb-4"><div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center"><div className="text-2xl font-bold text-green-700">{exitosos.length}</div><div className="text-xs text-green-600">Subidas correctamente</div></div><div className="bg-red-50 border border-red-200 rounded-xl p-4 text-center"><div className="text-2xl font-bold text-red-600">{fallidos.length}</div><div className="text-xs text-red-500">Con errores</div></div></div></div>)}
        </div>
  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 p-5 max-h-[90vh] flex flex-col">
        <div className="flex items-center justify-between mb-4"><h3 className="font-bold text-base">Buscar imagen en internet</h3><button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100"><X size={16} /></button></div>
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={() => buscar(query)} disabled={loading} className="px-4 py-2 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-60 flex items-center gap-2">{loading ? <Loader2 size={15} className="animate-spin" /> : <Search size={15} />} Buscar</button>
        </div>
        ...
        {!loading && results.length > 0 && (<div className="overflow-y-auto flex-1"><div className="grid grid-cols-3 sm:grid-cols-4 gap-3">{results.map((img, i) => (<button key={i} onClick={() => onSelect(img.url)} className="group relative rounded-xl overflow-hidden border-2 border-transparent hover:border-[#00a847] bg-gray-50 aspect-square flex items-center justify-center"><img src={img.url} alt={img.name} className="w-full h-full object-contain p-1" onError={e => { e.target.parentElement.style.display = "none"; }} /></button>))}</div></div>)}
...
function GestionCategorias({ categorias, onClose, onUpdated }) {
  ...
  const iniciarEdicion = (cat) => { setEditandoId(cat.id); setEditNombre(cat.nombre); };
  ...
  const guardarEdicion = async (id) => {
    ...
    setEditandoId(null); setEditNombre(""); onUpdated();
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm mx-4 p-6 max-h-[80vh] flex flex-col">
        ...
        <div className="flex gap-2 mb-4">
          <input type="text" value={nuevaNombre} onChange={e => setNuevaNombre(e.target.value)} onKeyDown={e => e.key === "Enter" && handleCrear()} placeholder="Nueva categoría..." className="flex-1 border rounded-xl px-4 py-2 text-sm focus:outline-none focus:border-[#00c254]" />
          <button onClick={handleCrear} disabled={saving || !nuevaNombre.trim()} className="px-3 py-2 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50">{saving ? <Loader2 size={15} className="animate-spin" /> : <Plus size={15} />}</button>
        </div>
        <div className="overflow-y-auto flex-1 space-y-1">
          ...
          {categorias.map(cat => (
            <div key={cat.id} className="flex items-center gap-2 p-2 rounded-xl hover:bg-gray-50 border border-gray-100 group">
              {editandoId === cat.id ? (<><input autoFocus type="text" value={editNombre} onChange={e => setEditNombre(e.target.value)} onKeyDown={e => { if (e.key === "Enter") guardarEdicion(cat.id); if (e.key === "Escape") cancelarEdicion(); }} className="flex-1 border border-[#00c254] rounded-lg px-2.5 py-1 text-sm focus:outline-none bg-[#edf7f2]" /><button onClick={() => guardarEdicion(cat.id)} className="p-1.5 text-green-600 hover:bg-green-50 rounded-lg flex-shrink-0"><Check size={14} /></button><button onClick={cancelarEdicion} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg flex-shrink-0"><X size={14} /></button></>) : (<><span className="flex-1 text-sm font-medium truncate">{cat.nombre}</span><button onClick={() => iniciarEdicion(cat)} className="p-1.5 text-gray-400 hover:text-[#00913f] hover:bg-[#edf7f2] rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"><Pencil size={13} /></button><button onClick={() => handleEliminar(cat)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0"><Trash2 size={13} /></button></>)}
            </div>
          ))}
        </div>
...
function CambiarPaginaPDFModal({ productos, categorias, onClose, onChanged }) {
  ...
  const [filtroPaginaActual, setFiltroPaginaActual] = useState("");
  const [seleccion, setSeleccion]             = useState(new Set());
  const [paginaDestino, setPaginaDestino]     = useState("");
  const [hojasOrden, setHojasOrden]           = useState([]);
  const [saving, setSaving]                   = useState(false);

  useEffect(() => {
    supabase.from("hojas_orden").select("nombre, posicion").order("posicion", { ascending: true })
      .then(({ data }) => setHojasOrden((data || []).map(h => h.nombre)));
  }, []);

  const paginasActuales = useMemo(() => {
    const set = new Set(productos.map(p => p.hoja_excel).filter(Boolean));
    return [...set].sort();
  }, [productos]);
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[90vh] flex flex-col gap-3">
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-3">
          ...
          <select value={paginaDestino} onChange={e => setPaginaDestino(e.target.value)} className="w-full border border-blue-300 rounded-xl px-4 py-2.5 text-sm bg-white font-semibold focus:outline-none focus:border-blue-500">
            <option value="">— Selecciona p&aacute;gina destino —</option>
        ...
        <div className="grid grid-cols-2 gap-2">
          ...
          <select value={filtroPaginaActual} onChange={e => setFiltroPaginaActual(e.target.value)} className="border rounded-xl px-3 py-2 text-sm">
            <option value="">Todas las páginas</option>
            <option value="__sin__">— Sin página —</option>
...
function CambiarGrupoModal({ productos, onClose, onChanged }) {
  ...
  const [saving, setSaving]                   = useState(false);
  const GRUPOS_OPTS = [{ value: "estacion", label: "🏪 Estación" }, { value: "cafeteria", label: "☕ Cafetería" }, { value: "ambos", label: "📦 Ambos" }, { value: "especifico", label: "🎯 Específicas" }];
  const [tiendas, setTiendas]               = useState([]);
  const [tiendasSeleccionadas, setTiendaSel] = useState(new Set());

  useEffect(() => {
    supabase.from("tiendas").select("id, nombre").eq("activa", true).neq("nombre", "PRINCIPAL").order("nombre").then(({ data }) => setTiendas(data || []));
  }, []);

  const categorias = [...new Map(productos.filter(p => p.categoria_id && p.categorias?.nombre).map(p => [p.categoria_id, { id: p.categoria_id, nombre: p.categorias.nombre }])).values()].sort((a, b) => a.nombre.localeCompare(b.nombre));
  ...
  const handleGuardar = async () => {
    ...
    if (grupoDestino === "especifico") {
      ...
      const filas = ids.flatMap(pid => [...tiendasSeleccionadas].map(tid => ({ producto_id: pid, tienda_id: tid })));
      if (filas.length) await supabase.from("producto_tiendas").insert(filas);
    } else { await supabase.from("producto_tiendas").delete().in("producto_id", ids); }
    try { Object.keys(localStorage).filter(k => k.startsWith("selogas_cat_")).forEach(k => localStorage.removeItem(k)); } catch {}
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[90vh] flex flex-col gap-3">
        <div className="flex items-center justify-between"><h2 className="font-bold text-lg flex items-center gap-2"><Layers size={18} /> Cambiar grupo masivo</h2><button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100"><X size={18} /></button></div>
        <div><label className="block text-xs font-semibold text-gray-600 mb-1">Asignar grupo a los seleccionados</label><div className="flex gap-2">{GRUPOS_OPTS.map(g => (<button key={g.value} onClick={() => setGrupoDestino(g.value)} className={`flex-1 py-2 rounded-xl border-2 text-sm font-bold transition-all ${grupoDestino === g.value ? "border-[#00913f] bg-[#edf7f2] text-[#00913f]" : "border-gray-200 text-gray-500 hover:border-gray-300"}`}>{g.label}</button>))}</div></div>
        <div className="grid grid-cols-2 gap-2">
          <select value={filtroCategoria} onChange={e => setFiltroCategoria(e.target.value)} className="border rounded-xl px-3 py-2 text-sm"><option value="">Todas las categorías</option>{categorias.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select>
          <div className="relative"><Search size={13} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar..." className="w-full border rounded-xl pl-8 pr-3 py-2 text-sm focus:outline-none focus:border-[#00913f]" /></div>
        </div>
        <div className="flex items-center justify-between"><span className="text-xs text-gray-500">{productosFiltrados.length} visibles · <span className="font-semibold text-[#00913f]">{seleccion.size} seleccionados</span>{seleccion.size > 0 && <button onClick={() => setSeleccion(new Set())} className="ml-2 text-red-400 hover:text-red-600 underline">limpiar</button>}</span><button onClick={toggleVisibles} className="text-xs text-[#00913f] hover:underline">{todosVisiblesSeleccionados ? "Deseleccionar visibles" : "Seleccionar visibles"}</button></div>
        <div className="overflow-y-auto flex-1 border rounded-xl divide-y min-h-[200px]">
          {productosFiltrados.length === 0 ? (<div className="flex items-center justify-center h-full text-gray-400 text-sm py-8">No hay productos</div>) : productosFiltrados.map(prod => (
            <label key={prod.id} className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer transition-colors ${seleccion.has(prod.id) ? "bg-[#edf7f2]" : "hover:bg-gray-50"}`}>
              <input type="checkbox" checked={seleccion.has(prod.id)} onChange={() => toggleSeleccion(prod.id)} className="rounded flex-shrink-0" />
              ...
              {prod.grupo_visualizacion && <span className={`text-xs px-2 py-0.5 rounded-full font-semibold flex-shrink-0 ${GRUPO_BADGE[prod.grupo_visualizacion] || "bg-gray-100 text-gray-500"}`}>{prod.grupo_visualizacion}</span>}
              {seleccion.has(prod.id) && <span className="text-[#00913f] flex-shrink-0">✓</span>}
            </label>
          ))}
        </div>
        {grupoDestino === "especifico" && (<div className="border rounded-xl p-3 bg-amber-50 border-amber-200"><p className="text-xs font-semibold text-amber-800 mb-2">🎯 Tiendas:</p><div className="grid grid-cols-2 gap-1.5 max-h-36 overflow-y-auto">{tiendas.map(t => (<label key={t.id} className="flex items-center gap-2 text-xs cursor-pointer hover:bg-amber-100 rounded-lg p-1.5"><input type="checkbox" checked={tiendasSeleccionadas.has(t.id)} onChange={() => { setTiendaSel(prev => { const next = new Set(prev); if (next.has(t.id)) next.delete(t.id); else next.add(t.id); return next; }); }} className="rounded flex-shrink-0" /><span className="truncate font-medium">{t.nombre}</span></label>))}</div></div>)}
        <div className="flex gap-3"><button onClick={onClose} className="flex-1 py-2.5 border rounded-xl text-sm font-medium hover:bg-gray-50">Cancelar</button><button onClick={handleGuardar} disabled={saving || !seleccion.size || !grupoDestino || (grupoDestino === "especifico" && tiendasSeleccionadas.size === 0)} className="flex-1 py-2.5 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50 flex items-center justify-center gap-2">{saving ? <Loader2 size={15} className="animate-spin" /> : <Check size={15} />}Asignar grupo {seleccion.size > 0 ? `(${seleccion.size})` : ""}</button></div>
      </div>
    </div>
  );
}
...
function MoverCategoriaModal({ productos, categorias, onClose, onMoved }) {
  ...
  const toggleVisibles = () => {
    ...
    setSeleccion(prev => { const next = new Set(prev); if (todosVisiblesSeleccionados) idsVisibles.forEach(id => next.delete(id)); else idsVisibles.forEach(id => next.add(id)); return next; });
  };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 p-6 max-h-[90vh] flex flex-col gap-3">
        <div className="flex items-center justify-between"><h2 className="font-bold text-lg flex items-center gap-2"><FolderOpen size={18} /> Mover productos</h2><button onClick={onClose} className="p-2 rounded-lg hover:bg-gray-100"><X size={18} /></button></div>
        <div className="grid grid-cols-2 gap-3">
          <div><label className="block text-xs font-semibold text-gray-600 mb-1">Filtrar por categoría</label><select value={origen} onChange={e => setOrigen(e.target.value)} className="w-full border rounded-xl px-3 py-2 text-sm"><option value="">Todas</option><option value="__sin__">Sin categoría</option>{categorias.map(c => <option key={c.id} value={c.id}>{c.nombre}</option>)}</select></div>
        ...
        <div className="relative"><Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" /><input type="text" value={busqueda} onChange={e => setBusqueda(e.target.value)} placeholder="Buscar por nombre o código..." className="w-full border rounded-xl pl-9 pr-9 py-2.5 text-sm focus:outline-none focus:border-[#00c254]" />{busqueda && <button onClick={() => setBusqueda("")} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600"><X size={14} /></button>}</div>
        <div className="flex items-center justify-between"><span className="text-xs text-gray-500">{productosFiltrados.length} visibles · <span className="font-semibold text-[#00913f]">{seleccion.size} seleccionados</span>{seleccion.size > 0 && <button onClick={() => setSeleccion(new Set())} className="ml-2 text-red-400 hover:text-red-600 underline">limpiar</button>}</span><button onClick={toggleVisibles} className="text-xs text-[#00913f] hover:underline">{todosVisiblesSeleccionados ? "Deseleccionar visibles" : "Seleccionar visibles"}</button></div>
        <div className="overflow-y-auto flex-1 border rounded-xl divide-y min-h-[200px]">
          {productosFiltrados.length === 0 ? (<div className="flex items-center justify-center h-full text-gray-400 text-sm py-8">No hay productos con ese criterio</div>) : productosFiltrados.map(prod => (
            <label key={prod.id} className={`flex items-center gap-2.5 px-3 py-2 cursor-pointer transition-colors ${seleccion.has(prod.id) ? "bg-[#edf7f2]" : "hover:bg-gray-50"}`}>
              <input type="checkbox" checked={seleccion.has(prod.id)} onChange={() => toggleSeleccion(prod.id)} className="rounded flex-shrink-0" />
              {prod.imagen_url && <img src={prod.imagen_url} alt="" className="w-8 h-8 object-contain rounded flex-shrink-0" />}
              <div className="flex-1 min-w-0"><p className="text-sm truncate font-medium">{prod.nombre}</p>{prod.codigo && <p className="text-xs text-gray-400 font-mono">{prod.codigo}</p>}</div>
              {seleccion.has(prod.id) && <span className="text-[#00a847] flex-shrink-0">✓</span>}
            </label>
          ))}
        </div>
        <div className="flex gap-3"><button onClick={onClose} className="flex-1 py-2.5 border rounded-xl text-sm font-medium hover:bg-gray-50">Cancelar</button><button onClick={handleMover} disabled={moving || !seleccion.size || !destino} className="flex-1 py-2.5 bg-[#00913f] text-white rounded-xl text-sm font-bold hover:bg-[#007a34] disabled:opacity-50 flex items-center justify-center gap-2">{moving ? <Loader2 size={15} className="animate-spin" /> : <ChevronRight size={15} />}Mover {seleccion.size > 0 ? `(${seleccion.size})` : ""}</button></div>
      </div>
    </div>
  );
}
...
function ProductoModal({ producto, categorias, onClose, onSave, modo }) {
  ...
  useEffect(() => {
    supabase.from("tiendas").select("id, nombre").eq("activa", true).neq("nombre", "PRINCIPAL").order("nombre").then(({ data }) => setTiendas(data || []));
    supabase.from("hojas_orden").select("nombre, posicion").order("posicion", { ascending: true }).then(({ data }) => setHojasOrden((data || []).map(h => h.nombre)));
    if (producto?.id && producto?.grupo_visualizacion === "especifico") {
      supabase.from("producto_tiendas").select("tienda_id").eq("producto_id", producto.id).then(({ data }) => { if (data?.length) setTiendasSelec(new Set(data.map(r => r.tienda_id))); });
    }
  }, []);
  ...
  const handleUpload = async (e) => {
    ...
    if (!error) { const { data: url } = supabase.storage.from('imagenes').getPublicUrl(path); setForm(f => ({ ...f, imagen_url: url.publicUrl })); }
    setUploading(false);
  };

  const handleSave = async () => {
    if (!form.nombre.trim()) { alert('El nombre es obligatorio'); return; }
    if (form.grupo_visualizacion === 'especifico' && tiendasSelec.size === 0) { alert('Selecciona al menos una tienda específica.'); return; }
    setSaving(true);
    const payload = { ...form, activo: true, categoria_id: form.categoria_id || null, hoja_excel: form.hoja_excel || null, grupo_visualizacion: form.grupo_visualizacion === 'ambos' ? 'ambas' : form.grupo_visualizacion };
    let prodId = producto?.id;
    if (modo === 'crear') {
      const { data, error } = await supabase.from('productos').insert([payload]).select().single();
      if (error) { setSaving(false); alert('Error al crear: ' + error.message); return; }
      prodId = data.id; await guardarTiendasEspecificas(prodId); setSaving(false); onSave(data, 'crear');
    } else {
      ...
      await guardarTiendasEspecificas(prodId); setSaving(false); onSave({ ...producto, ...payload }, 'editar');
    }
    try { Object.keys(localStorage).filter(k => k.startsWith("selogas_cat_")).forEach(k => localStorage.removeItem(k)); } catch {}
  };
  ...
  const guardarTiendasEspecificas = async (prodId) => {
    ...
    if (form.grupo_visualizacion === "especifico" && tiendasSelec.size > 0)
      await supabase.from("producto_tiendas").insert([...tiendasSelec].map(tid => ({ producto_id: prodId, tienda_id: tid })));
  };
  const toggleTienda = (id) => setTiendasSelec(prev => { const next = new Set(prev); if (next.has(id)) next.delete(id); else next.add(id); return next; });
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/40">
        <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md mx-4 p-6 max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-semibold text-gray-700 mb-1">P&aacute;gina PDF</label>
              <select value={form.hoja_excel} onChange={e => setForm(f => ({...f, hoja_excel: e.target.value}))} className="w-full border rounded-xl px-4 py-2.5 text-sm">
                <option value="">— Sin asignar —</option>
                {hojasOrden.map(h => <option key={h} value={h}>{h}</option>)}
              </select>
              <p className="text-xs text-gray-400 mt-1">P&aacute;gina donde aparecer&aacute; en el PDF del pedido</p>
            </div>
            <div><label className="block text-sm font-semibold text-gray-700 mb-1">Descripci&oacute;n</label><textarea value={form.descripcion} onChange={e => setForm(f => ({...f, descripcion: e.target.value}))} rows={2} className="w-full border rounded-xl px-4 py-2.5 text-sm resize-none" /></div>
            <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">{GRUPOS.map(g => <button key={g.value} onClick={() => setForm(f => ({...f, grupo_visualizacion: g.value}))} className={`flex flex-col items-center p-3 rounded-xl border-2 text-center transition-all ${form.grupo_visualizacion === g.value ? 'border-[#00913f] bg-[#edf7f2]' : 'border-gray-200 hover:border-gray-300'}`}><span className="text-sm font-bold">{g.label}</span><span className="text-xs text-gray-500 mt-0.5">{g.desc}</span></button>)}</div>
            </div>
            {form.grupo_visualizacion === 'especifico' && (
              <div className="border-2 border-amber-200 bg-amber-50 rounded-xl p-3">
                <p className="text-xs font-bold text-amber-800 mb-2">🎯 Tiendas que pueden ver este producto</p>
                <div className="grid grid-cols-2 gap-1 max-h-40 overflow-y-auto">{tiendas.map(t => (<label key={t.id} className={`flex items-center gap-2 text-xs cursor-pointer rounded-lg p-1.5 transition-colors ${tiendasSelec.has(t.id) ? "bg-amber-100 font-semibold text-amber-900" : "hover:bg-amber-100 text-gray-700"}`}><input type="checkbox" checked={tiendasSelec.has(t.id)} onChange={() => toggleTienda(t.id)} className="rounded flex-shrink-0" /><span className="truncate">{t.nombre}</span></label>))}</div>
              </div>
            )}
            <div>
