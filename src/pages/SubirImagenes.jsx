  ...
  return (
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/60">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 p-5 max-h-[90vh] flex flex-col">
        ...
        {error && (<div className="mb-3 p-3 bg-amber-50 rounded-xl"><p className="text-sm text-amber-700 mb-2">{error}</p><a href={`https://www.google.com/search?q=${encodeURIComponent(query + ' imagen')}&tbm=isch`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-amber-300 rounded-lg text-sm text-amber-700 font-semibold hover:bg-amber-50"><Globe size={13} /> Abrir Google Images</a></div>)}
        {loading && (
        ...
        {!loading && !results.length && !error && (
          <div className="flex flex-col items-center justify-center text-gray-400 py-6">
            ...
            <a href={`https://www.google.com/search?q=${encodeURIComponent(query + ' imagen producto')}&tbm=isch`} target="_blank" rel="noreferrer" className="flex items-center gap-2 px-4 py-2 bg-blue-50 border border-blue-200 rounded-xl text-sm text-blue-700 font-semibold hover:bg-blue-100"><Globe size={14} /> Buscar en Google Images</a>
