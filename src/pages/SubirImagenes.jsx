      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl mx-4 p-5 max-h-[90vh] flex flex-col">
        ...
        <div className="mb-3">
          ...
          <div className="flex gap-2">
            <input type="text" value={manualUrl} onChange={e => setManualUrl(e.target.value)}
              placeholder="https://..." className="flex-1 border rounded-xl px-4 py-2 text-sm" />
        ...
        {error && (
          <div className="mb-3 p-3 bg-amber-50 rounded-xl">
            ...
            <a href={`https://www.google.com/search?q=${encodeURIComponent(query + ' imagen')}&tbm=isch`} target="_blank" rel="noreferrer"
              className="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-amber-300 rounded-lg text-sm text-amber-700 font-semibold hover:bg-amber-50">
              <Globe size={13} /> Abrir Google Images
            </a>
          </div>
        )}
...
// ── Tarjeta de producto con botón eliminar en hover ──────────────────
function TarjetaProducto({ prod, imagenActual, estaGuardando, recienGuardado, bulkActivo, onBuscar, onEliminar }) {
  ...
  return (
    ...
    >
      <div className="relative h-28 bg-gray-50 flex items-center justify-center">
        ...
        {/* Tick verde — recién guardada */}
        {recienGuardado && (
          <div className="absolute top-1.5 right-1.5 bg-green-500 rounded-full p-0.5 pointer-events-none">
        ...
        <div className="mt-auto">
          ...
          >
            {estaGuardando ? (
              ...
              <><Globe size={12} /> {imagenActual ? "Cambiar" : "Buscar"}</>
            )}
          </button>
        </div>
      </div>
...
export default function SubirImagenes() {
  ...
  const handleSeleccionCarpeta = async (e) => {
    ...
    const mapaProductos = {};
    prods.forEach(p => { if (p.codigo) mapaProductos[p.codigo.toLowerCase().trim()] = p; });
    const imagenesValidas = files.filter(f => f.type.startsWith("image/") || /\.(jpg|jpeg|png|gif|webp|avif|svg)$/i.test(f.name));
    const emparejamientos = imagenesValidas.map(file => {
      ...
      return { file, nombreArchivo: file.name, codigoBuscado: nombreSinExt, producto: productoEncontrado || null, previewUrl: URL.createObjectURL(file), estado: productoEncontrado ? "listo" : "sin_match" };
    });
  ...
  const handleSubir = async () => {
    ...
    setSubiendo(true); setFase("subiendo"); setProgreso(0);
    const res = [];
    for (let i = 0; i < listos.length; i++) {
      ...
      try {
        ...
        const { error: uploadError } = await supabase.storage.from("imagenes").upload(fileName, item.file, { upsert: true, contentType: item.file.type });
    ...
    setResultados(res); setSubiendo(false); setFase("hecho");
  };
  ...
  useEffect(() => {
    if ((tab === "internet" || tab === "eliminar") && categorias.length === 0) {
                  ...
                  <div className="divide-y divide-gray-50">
                    {fallidos.map((item, i) => (
                      <div key={i} className="flex items-center gap-3 px-4 py-2.5">
                        <div className="w-10 h-10 rounded-lg bg-gray-100 flex items-center justify-center overflow-hidden flex-shrink-0">{item.previewUrl ? <img src={item.previewUrl} alt="" className="w-full h-full object-contain" /> : <Package size={16} className="text-gray-300" />}</div>
                        <div className="flex-1 min-w-0"><p className="text-sm font-semibold text-gray-700 truncate">{item.nombreArchivo}</p><p className="text-xs text-red-500">{item.error}</p></div>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {bulkLog.length > 0 && (
                <div className="mt-2 max-h-24 overflow-y-auto space-y-0.5">
                  {bulkLog.slice(0, 8).map((entry, i) => (
                    <div key={i} className={`text-xs flex items-center gap-1.5 ${entry.exito ? "text-green-700" : "text-red-500"}`}>
                      {entry.exito ? "✓" : "✗"} <span className="truncate">{entry.nombre}</span>
                      {!entry.exito && <span className="text-gray-400 flex-shrink-0">({entry.error})</span>}
                    </div>
                  ))}
                </div>
              )}
                ...
                {productosFiltrados.map(prod => {
                  ...
                  return (
                    <TarjetaProducto
                      ...
                      bulkActivo={bulkActivo}
                      onBuscar={() => setBuscandoPanel(prod)}
                      onEliminar={() => setConfirmEliminar({ tipo: "producto", productoId: prod.id, productoNombre: prod.nombre })}
                    />
                  );
                })}
              </div>
