...
import { Table2, Loader2, RefreshCw, Search, ChevronDown, ChevronUp, Save, X } from "lucide-react";
...
export default function ExcelSelogas() {
  ...
  const hojas = (() => {
    ...
    const nombresOrdenados = [
      ...ORDEN_HOJAS.filter(h => mapaHojas[h]),
      ...Object.keys(mapaHojas).filter(h => !ORDEN_HOJAS.includes(h)).sort(),
    ];
  ...
  return (
    <div style={{ maxWidth: "100%", fontFamily: "var(--font-sans)" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: "12px", marginBottom: "14px" }}>
        <div>
          ...
          <p style={{ fontSize: "13px", color: "var(--color-text-secondary)", margin: "4px 0 0" }}>
            {productos.length} productos · {hojas.length} hojas
            {cambios > 0 && (
        ...
        >
          {loading
            ? <Loader2 size={14} style={{ animation: "xspin 1s linear infinite" }} />
            : <RefreshCw size={14} />
          }
          Recargar
        </button>
      ...
      {/* Leyenda */}
      <div style={{ display: "flex", gap: "20px", marginBottom: "14px", flexWrap: "wrap", fontSize: "11px", color: "var(--color-text-secondary)" }}>
        <span>💡 Clic en el <strong>número de fila</strong> (junto al código) para cambiar <code>orden_excel</code></span>
        <span>💡 Clic en <strong>A/B · D/E · G/H</strong> para mover el producto a otra columna</span>
      </div>
