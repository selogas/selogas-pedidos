...
function GestionNovedades() {
  ...
  const handleImageUpload = async (e) => {
    ...
    const { error } = await supabase.storage.from('novedades').upload(nombre, file, { upsert: false });
    if (error) {
      setErrorMsg('Error al subir: ' + error.message);
    } else {
      const url = SUPABASE_URL + '/storage/v1/object/public/novedades/' + nombre;
      setForm(function(f) { return Object.assign({}, f, { imagen_url: url }); });
    }
    setSubiendo(false);
  ...
  const handleSubmit = async (e) => {
    ...
    if (editandoId) {
      ...
      await supabase.from('novedades').insert([Object.assign({}, form, { orden: novedades.length })]);
  ...
  const onDragEnd = async () => {
    if (dragItem.current !== null && overIdx !== null && dragItem.current !== overIdx) {
      ...
      const moved = reord.splice(dragItem.current, 1)[0];
      reord.splice(overIdx, 0, moved);
      setNovedades(reord);
      await Promise.all(reord.map(function(item, i) {
        return supabase.from('novedades').update({ orden: i }).eq('id', item.id);
      }));
  ...
  const btnSubirLabel = subiendo ? 'Subiendo imagen...' : 'Subir imagen desde tu dispositivo';
    ...
    <div className="space-y-4">
      <form onSubmit={handleSubmit} className="bg-gray-50 rounded-2xl border border-gray-200 p-5 space-y-3">
        ...
        <div>
          ...
          >
            {subiendo
              ? <Loader2 size={15} className="animate-spin text-[#00913f]" />
              : <Plus size={15} className="text-[#00913f]" />
            }
            <span>{btnSubirLabel}</span>
          </button>
          <input
            ref={fileInputRef}
            type="file"
          ...
          <input
            ...
            onChange={function(e) { setForm(Object.assign({}, form, { imagen_url: e.target.value })); }}
          />
          ...
          {form.imagen_url ? (
            <div className="mt-2 relative">
              ...
              <button
                type="button"
                onClick={function() { setForm(Object.assign({}, form, { imagen_url: '' })); }}
        ...
        <div>
          ...
          <input
            ...
            value={form.titulo}
            onChange={function(e) { setForm(Object.assign({}, form, { titulo: e.target.value })); }}
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            onChange={function(e) { setForm(Object.assign({}, form, { descripcion: e.target.value })); }}
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            placeholder="https://..."
            value={form.enlace}
            onChange={function(e) { setForm(Object.assign({}, form, { enlace: e.target.value })); }}
          />
        </div>
        ...
        <label className="flex items-center gap-2 text-sm text-gray-700 cursor-pointer">
          <input
            type="checkbox"
            checked={form.activa}
            onChange={function(e) { setForm(Object.assign({}, form, { activa: e.target.checked })); }}
            className="rounded"
          />
          Visible en la app
        </label>

        {errorMsg ? <p className="text-red-600 text-xs">{errorMsg}</p> : null}
        {exito ? <p className="text-[#00913f] text-xs font-semibold">{exito}</p> : null}
        ...
        <div className="flex gap-2 pt-1">
          <button
            ...
            style={{ background: '#00913f', opacity: (guardando || subiendo) ? 0.7 : 1 }}
          >
            {guardando ? <Loader2 size={15} className="animate-spin" /> : <Save size={15} />}
            <span>{editandoId ? 'Guardar cambios' : 'Añadir'}</span>
          </button>
          {editandoId ? (
            <button
              type="button"
              ...
              return (
                ...
                >
                  ...
                  <img
                    ...
                    onError={function(e) { e.target.src = 'data:image/svg+xml,<svg xmlns="http://www.w3.org/2000/svg" width="56" height="36"><rect fill="%23eee" width="56" height="36"/></svg>'; }}
                  />
                  <div className="flex-1 min-w-0">
                    {nov.titulo ? (
                      <p className="text-sm font-semibold text-gray-800 truncate">{nov.titulo}</p>
                    ) : (
                      <p className="text-sm text-gray-400 italic truncate">Sin título</p>
                    )}
                  </div>
                  <div className="flex items-center gap-1 flex-shrink-0">
                    <button
                      onClick={function() { toggleActiva(nov); }}
                    ...
                    <button
                      onClick={function() { iniciarEdicion(nov); }}
                      className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-gray-100 transition-colors"
                      title="Editar"
                    >
                      <span>✏️</span>
                    ...
                    <button
                      onClick={function() { eliminar(nov.id); }}
...
export default function Configuracion() {
  ...
  useEffect(() => {
    configuracionApi.list().then(function(configs) {
      const vals = {};
      configs.forEach(function(c) { vals[c.clave] = c.valor || ''; });
      setValores(vals);
      setLoading(false);
    });
  }, []);
  ...
  const handleGuardar = async (clave) => {
    setGuardando(function(g) { return Object.assign({}, g, { [clave]: true }); });
    await configuracionApi.upsert(clave, valores[clave] || '');
    setGuardando(function(g) { return Object.assign({}, g, { [clave]: false }); });
    setExito(function(e) { return Object.assign({}, e, { [clave]: true }); });
    setTimeout(function() {
      setExito(function(e) { return Object.assign({}, e, { [clave]: false }); });
    }, 2000);
  };
              ...
              <div className="flex gap-3 mt-3">
                {type === 'textarea' ? (
                  <textarea
                    ...
                    value={valores[clave] || ''}
                    onChange={function(e) { setValores(function(v) { return Object.assign({}, v, { [clave]: e.target.value }); }); }}
                  />
                ) : (
                  <input
                    type="text"
                ...
                <button
                  onClick={function() { handleGuardar(clave); }}
                  ...
                  style={{ background: exito[clave] ? '#22c55e' : '#00913f', opacity: guardando[clave] ? 0.7 : 1 }}
                >
                  {guardando[clave]
                  ...
                  <span>{exito[clave] ? 'Guardado' : 'Guardar'}</span>
                </button>
              </div>
            </div>
          );
      ...
      <div className="bg-white rounded-2xl border border-gray-200 p-6">
        <div className="flex items-start gap-3 mb-5">
          ...
          <div>
            ...
            <div className="text-xs text-gray-400 mt-0.5">Imágenes del carrusel que aparecen en la página de Inicio</div>
          </div>
        </div>
        <GestionNovedades />
      </div>
